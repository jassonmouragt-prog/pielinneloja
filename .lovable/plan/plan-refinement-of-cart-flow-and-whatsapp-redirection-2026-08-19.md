...
- **Wait**: The user wants to register the sale _only_ when it's sent to WhatsApp. However, `window.open` is a client-side action that we can't fully guarantee "sent" (as it just opens the app).
- **Refinement**: I will move the `registerSale` call to happen _after_ the `window.open` call or keep it right before but ensure the name validation is robust and provides immediate feedback if missing.
