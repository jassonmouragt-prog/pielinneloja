    ...
    queryFn: async () => {
      ...
      const chartData = months.map((month) => {
        ...
        const revenue = monthSales.reduce(
          (acc: number, sale: any) => acc + Number(sale.total_amount),
          0,
        );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground pt-1">
              {billingData?.currentMonth.count || 0} vendas confirmadas
            </p>
          </CardContent>
        </Card>
        ...
        <Card
          className={
            (billingData?.currentMonth.profit ?? 0) < 0 ? "border-red-200" : "border-emerald-200"
          }
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {(billingData?.currentMonth.profit ?? 0) >= 0 ? "Lucro (Mês)" : "Prejuízo (Mês)"}
            </CardTitle>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${(billingData?.currentMonth.profit ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}
            >
            ...
            <div className="flex items-center pt-1">
              {billingData && billingData.growth >= 0 ? (
                <span className="text-xs font-medium text-green-600 flex items-center">
                  <ArrowUpRight className="mr-1 h-3 w-3" />+{billingData.growth.toFixed(1)}% vs mês
                  anterior
                </span>
              ) : (
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              R${" "}
              {billingData?.currentMonth.count
                ? (billingData.currentMonth.revenue / billingData.currentMonth.count).toFixed(2)
                : "0.00"}
            </div>
      ...
      <Card>
        ...
        <CardContent className="pl-2">
          <div className="h-[350px] w-full">
            <ClientOnly
              fallback={<div className="h-full w-full bg-gray-100 animate-pulse rounded-md" />}
            >
        ...
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <div className="text-right text-xs text-muted-foreground">
            <span className="text-green-600 font-bold">
              +R$ {billingData?.totalRevenue.toFixed(2)}
            </span>
            <span className="mx-1">·</span>
            <span className="text-red-600 font-bold">
              -R$ {billingData?.totalExpenses.toFixed(2)}
            </span>
            <span className="mx-1">·</span>
