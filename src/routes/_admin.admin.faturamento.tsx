...
function BillingPage() {
  ...
  const { data: billingData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const sales = await getBillingFn();
      ...
      const chartData = months.map((month) => {
        ...
        return {
          ...
          total: monthSales.reduce((acc: number, sale: any) => acc + Number(sale.total_amount), 0),
      ...
      <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="flex items-center pt-1">
              {billingData && billingData.growth >= 0 ? (
                <span className="text-xs font-medium text-green-600 flex items-center">
                  <ArrowUpRight className="mr-1 h-3 w-3" />+{billingData.growth.toFixed(1)}% em
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground pt-1">Pedidos finalizados com sucesso</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              {billingData?.currentMonth.count
                ? (billingData.currentMonth.total / billingData.currentMonth.count).toFixed(2)
                : "0.00"}
            </div>
            <p className="text-xs text-muted-foreground pt-1">Ticket médio deste mês</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <Bar dataKey="total" radius={[4, 4, 0, 0]}>
                    {billingData?.chartData.map((entry: any, index: number) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          index === (billingData?.chartData.length || 0) - 1
                            ? "#E84688"
                            : "#F0629280"
                        }
                      />
